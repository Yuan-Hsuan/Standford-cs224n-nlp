# 02 · Count-Based Word Vectors 計數式詞向量 — co-occurrence + SVD 共現 + SVD

> **Source 出處**: CS224n Lecture 1 · Assignment 1 Part 1.
> **How to read 讀法**: §1–§2 basics → **§3–§5 the core (theorem + input + meaning)** → §7 how to compute → §9 code.
...
4. The input: what is a co-occurrence matrix? 什麼是共現矩陣
5. ⭐ Physical meaning of U / S / V 三矩陣的物理意義
6. Geometric intuition 幾何直覺
7. Compute S + worked example 怎麼算 S + 手把手例子
8. Truncated SVD 截斷降維
9. a1 code a1 程式碼 (§9.1 `n_iter`)
10. One-line summary 一句話總結
## 1. One-line overview 總覽

**SVD factors any matrix $A$ into three matrices**, exposing its "most important directions" so we can
reduce dimensions. 把任意矩陣拆成三個,找出「最重要的方向」來降維。
...
### 2.1 Definition 定義
For a **square** matrix $A$, if $A\vec{v}=\lambda\vec{v}$ for some nonzero $\vec{v}$ and scalar $\lambda$,
then $\lambda$ is an **eigenvalue** and $\vec{v}$ an **eigenvector**.
...
**Compare the spectral theorem** 對照譜定理: any **symmetric** $M$ diagonalizes as $M=Q\Lambda Q^T$, with
$Q$'s columns = orthonormal eigenvectors, $\Lambda$ = eigenvalues. Match term-by-term:

$$\underbrace{AA^T}_{M} = \underbrace{U}_{Q}\,\underbrace{(SS^T)}_{\Lambda}\,\underbrace{U^T}_{Q^T}$$

1. **$U$** = orthonormal eigenvectors of $AA^T$. Orthonormal is *guaranteed* because $AA^T$ is symmetric
   ($(AA^T)^T=AA^T$) — orthogonality is a gift of symmetry, not forced. 對稱⇒正交是保證的。
...
**Check with §7** 驗證: $AA^T=\begin{pmatrix}17&8\\8&17\end{pmatrix}$ has $\lambda=25,9$ →
$\sigma=\sqrt{25},\sqrt9=5,3$ ✅

## 4. The input: what *is* a co-occurrence matrix? 什麼是共現矩陣

Before decoding $U/S/V$, be clear on the $A$ we feed in. A **co-occurrence matrix** counts, for every
word, which words appear near it. Slide a fixed **window** of ±$n$ words across the corpus; entry
$M_{ij}$ = **how many times word $j$ falls inside the window of word $i$**. 用一個 ±n 的視窗掃過語料,
數「詞 $j$ 出現在詞 $i$ 附近幾次」,填進矩陣。

- **Word × word** (a1's version): symmetric, shape `(num_words × num_words)` — "context" *is* just the
  other words in the window, so rows and columns share one vocabulary. a1 還會在每句前後加
  `<START>` / `<END>`,並用視窗大小 `window_size`。
- **Why counts carry meaning** 為何數數就有語義: *"You shall know a word by the company it keeps."* Words
  used in similar contexts end up with **similar rows** → *doctor* ≈ *nurse*, *doctor* ≠ *banana*。

**Tiny example** 最小例子 — corpus `"I enjoy learning"`, window $n=1$:
...
Each **row is that word's raw vector**. The catch 問題: it's **high-dimensional and sparse** (length =
vocab size, ~10⁴, mostly zeros). So we run **SVD on $M$** to squeeze it into short, dense vectors — which
is exactly what the rest of this note does. 又大又稀疏,所以用 SVD 壓成低維稠密向量。

## 5. ⭐ Physical meaning of U / S / V 三矩陣的物理意義

Now take that **co-occurrence matrix** as $A$ (`word(row) × context(col)`), and think of SVD as
**discovering hidden latent topics between words and contexts**. 在詞和上下文之間,挖出隱藏的「語義主題」。
...
> $U$ gives direction; $\times S$ scales each axis by that topic's importance ($\sigma_i$). Kirk Baker's
> tutorial computes word similarity on rows of **$US$** (contexts on rows of $VS$). That's why a1's
> `reduce_to_k_dim` docstring says it "actually returns **U * S**". 相似度在 $US$ 的列上算。

**Linking sentence 串起來**: $U$ and $V$ are the **same latent topics** on the word side vs the context
side; $S$ is each topic's **weight**. → a three-layer **word — topic — context** structure.
...
## 6. Geometric intuition: rotate → stretch → rotate 幾何直覺
...
**directions = columns of $U$**. 單位圓→橢圓:半軸長=奇異值,方向=U 的列。 This is *why* a big singular value
= more info: it's the direction stretched most. 拉最長=資訊最多。
...
### 9.1 What is `n_iter`? (a1 sets `n_iters = 10`) `n_iter` 是什麼

`TruncatedSVD` uses **randomized SVD**: instead of exact full SVD, it randomly projects the matrix and
runs a few **power iterations** to approximate the top-$k$ components. `n_iter` = how many power
iterations. 冪次迭代次數。

- More `n_iter` → more accurate, slower; fewer → faster, less precise. 多=準但慢,少=快但糙。
- Why 10 (sklearn default 5)? Co-occurrence matrices have **slowly-decaying** singular values, so a few
  extra iterations stabilize the result. 奇異值衰減慢,多迭代幾輪更穩。
- It trades **accuracy ↔ speed**; does **not** change the output shape. 只影響精度/速度,不改形狀。

### 9.2 Singular vectors have an arbitrary sign 奇異向量的正負號是任意的

If $u$ is a singular vector, so is $-u$ — both are valid. Randomized `TruncatedSVD` may return either
sign, so a 2-D plot can come out **mirrored or rotated 180°** vs a reference figure. 奇異向量正負號任意,
所以畫出來的圖可能左右/上下翻轉,這不是 bug。 **Only relative positions (which words cluster) are
meaningful; the axis orientation is not.** 有意義的是相對位置與分群,座標軸方向沒有意義。

## 10. One-line summary 一句話總結

> **SVD factors $A=USV^T$**: $U$ = words' directions on latent topics, $V$ = contexts' directions, $S$ =
> each topic's importance ($\sigma=\sqrt{\lambda}$ of $A^TA$). The word vectors are the rows of $U\cdot S$
> (not $U$ alone). Keep the top $k$ topics → drop from ~10⁴ dims to $k$ while keeping meaning.
